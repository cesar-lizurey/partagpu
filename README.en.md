...
- [examples/discover_gpu.ipynb](examples/discover_gpu.ipynb) — a Jupyter notebook to see PartaGPU in action: discovering the room's GPUs, running a command on a peer, multi-machine DDP training, and a tiny colorisation UNet trained in distributed mode (French version: [examples/decouverte_gpu.ipynb](examples/decouverte_gpu.ipynb))
...
├── examples/                    # Notebooks + example scripts + smoke tests
│   ├── discover_gpu.ipynb       # example notebook (English)
│   ├── decouverte_gpu.ipynb     # example notebook (French)
│   ├── ddp_train_demo.py
│   └── smoke_*.py
├── docs/
...
For the [examples/](examples/) folder, there's a full `requirements.txt` + Jupyter kernel setup — see [examples/discover_gpu.ipynb](examples/discover_gpu.ipynb) (English) or [examples/decouverte_gpu.ipynb](examples/decouverte_gpu.ipynb) (French).
